...
class SHMEMSEG(object):
    ...
    def connect_segment(self):
        ...
        if self.initialize:
            self.set(opt='default')
            self.initialize = False

    def set(self, val=None, opt='remain'):
        """
        Set the data to the shared memory.
        :param val: {name: data}
        :param opt: 'remain'  if keeping  the rest unchanged
                    'update'  if updating the rest from shared memory
                    'default' if setting  the rest to default
        """
        if val is None:
            val = []
        ...
        if opt == 'remain':
            ...
            for idx in range(len(self.blocks)):
                if self.blocks[idx]['name'] in val:
                    ...
                    self.blocks[idx]['data'] = self.blocks[idx]['default']
            self.update_segment()
            with self.mem_lock:
                self.mem_addr.seek(0)
                self.mem_addr.write(self.mem_data.data)
