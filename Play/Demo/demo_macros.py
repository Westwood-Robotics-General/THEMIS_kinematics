...
bx_offset = {WALK: +0.000, RUN: -0.010, JUMP: +0.00} if HARDWARE else {WALK: +0.000, RUN: +0.000, JUMP: +0.00}
by_offset = {WALK: +0.000, RUN: -0.020, JUMP: +0.00} if HARDWARE else {WALK: +0.000, RUN: +0.000, JUMP: +0.00}

# CoM position adjustment in standing [m]
px_offset = -0.005 if HARDWARE else 0.000

# TOP-LEVEL
# velocity limit in moving [m]
COM_VELOCITY_LIMIT = {WALK: [+0.20, -0.20, +0.10, -0.10],  # [x_max, x_min, y_max, y_min]
                      RUN:  [+0.20, -0.20, +0.10, -0.10]} if HARDWARE else {WALK: [+0.40, -0.20, +0.20, -0.20],
                                                                            RUN:  [+0.60, -0.30, +0.20, -0.20]}
...
PARAMETER_MAX          = [ 0.010,  0.020,  0.020,       8,    10,    20,    0.10,  0.10,    30,      10,    10,   0.05,       5]
PARAMETER_MIN          = [-0.010, -0.020, -0.030,      -8,   -10,   -20,   -0.10, -0.10,   -30,     -10,   -10,  -0.05,       0]
