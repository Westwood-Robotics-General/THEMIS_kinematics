...
def set_joint_positions(robot, npt, dt,
    ...
    if leg_move:
        ...
        traj_jnt_hip_pitch_r   = np.linspace(robot.joint[HIP_PITCH_R]['q'],   leg_goal_positions[1], npt, endpoint=True)
        traj_jnt_hip_roll_r    = np.linspace(robot.joint[HIP_ROLL_R]['q'],    leg_goal_positions[2], npt, endpoint=True)
        ...
        traj_jnt_hip_pitch_l   = np.linspace(robot.joint[HIP_PITCH_L]['q'],   leg_goal_positions[6], npt, endpoint=True)
        traj_jnt_hip_roll_l    = np.linspace(robot.joint[HIP_ROLL_L]['q'],    leg_goal_positions[7], npt, endpoint=True)
...
if __name__ == '__main__':
    ...
    if mode == 'n':
        ...
        # leg pose
        lr1, lr2, lr3, lr4, lr5 = 0., 0., 0., 0., 0.
        ll1, ll2, ll3, ll4, ll5 = 0., 0., 0., 0., 0.
    else:
        # arm pose
        ...
        # leg pose
        bpr = np.array([0.07, -0.07, -0.40])  # right foot position  in body frame
        bpl = np.array([0.07, +0.07, -0.40])  # left  foot position  in body frame
        bxr = np.array([1., 0., 0.])             # right foot direction in body frame
        bxl = np.array([1., 0., 0.])             # left  foot direction in body frame
        
        qr = Bruce.solve_leg_IK(leg="right", x=bxr, p=bpr, q0=np.zeros(5))
        ql = Bruce.solve_leg_IK(leg="left",  x=bxl, p=bpl, q0=np.zeros(5))
        lr1, lr2, lr3, lr4, lr5 = qr[0], qr[1], qr[2], qr[3], qr[4]
        ll1, ll2, ll3, ll4, ll5 = ql[0], ql[1], ql[2], ql[3], ql[4]
