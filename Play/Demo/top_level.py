...
class UserCommand(object):
    ...
    def operate(self):
        if self.controller == 'keyboard':
            ...
            if Bruce.gamepad['LZ'] and Bruce.gamepad['RZ']:
                os.system("gnome-terminal -- bash -c 'pkill screen; kill -9 $(pgrep sim_bootup)'")
        ...
        if self.target_mode == BALANCE:
            if self.mode in [WALK, RUN]:
                self.in_recover = True
        elif self.target_mode in [WALK, RUN]:
            if self.mode == BALANCE:
                self.in_recover = True
            elif self.mode in [WALK, RUN]:
                ...
                self.target_mode = BALANCE
        elif self.target_mode == JUMP:
            if self.mode == BALANCE:
                self.mode = JUMP
            elif self.mode == JUMP:
                self.display()
                time.sleep(1) if HARDWARE else time.sleep(2)
                self.mode = BALANCE
                self.target_mode = BALANCE
    ...
    def set_command(self):
        ...
        if self.mode in [WALK, RUN, JUMP]:
            input_data['dcm_offset_compensation'] = np.array([bx_offset[self.mode], by_offset[self.mode]])
    ...
    def display(self):
        ...
        print('_________')
        print('')
        print('IN', self.mode_name[self.mode], '(press SPACEBAR + 0/1/2/3 to balance/walk/run/jump)')
        print('_________')
        print('')
        if self.mode == BALANCE:
