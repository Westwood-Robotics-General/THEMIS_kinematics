...
'''
Compile BRUCE dynamics ahead of time (AOT) using Numba
'''
...
r_12_hat = MF.hat(COORDINATE_VECTOR[HIP_PITCH_R])
r_23_hat = MF.hat(COORDINATE_VECTOR[HIP_ROLL_R])
...
r_67_hat  = MF.hat(COORDINATE_VECTOR[HIP_PITCH_L])
r_78_hat  = MF.hat(COORDINATE_VECTOR[HIP_ROLL_L])
...
Is_2  = SPATIAL_INERTIA[HIP_PITCH_R]
Is_3  = SPATIAL_INERTIA[HIP_ROLL_R]
Is_4  = SPATIAL_INERTIA[KNEE_PITCH_R]
Is_5  = SPATIAL_INERTIA[ANKLE_PITCH_R]

Is_6  = SPATIAL_INERTIA[HIP_YAW_L]
Is_7  = SPATIAL_INERTIA[HIP_PITCH_L]
Is_8  = SPATIAL_INERTIA[HIP_ROLL_L]
...
@cc.export('robot_ID', '(f8[:,:], f8[:], f8[:], f8[:],'
                       ' f8[:],   f8[:],'
                       ' f8[:],   f8[:])')
def robot_ID(R, p, w, bv,
              q_leg_r,  q_leg_l,
             dq_leg_r, dq_leg_l):
    
    q1, q2, q3, q4, q5  = q_leg_r[0], q_leg_r[1], q_leg_r[2], q_leg_r[3], q_leg_r[4]
    q6, q7, q8, q9, q10 = q_leg_l[0], q_leg_l[1], q_leg_l[2], q_leg_l[3], q_leg_l[4]
    dq1, dq2, dq3, dq4, dq5  = dq_leg_r[0], dq_leg_r[1], dq_leg_r[2], dq_leg_r[3], dq_leg_r[4]
    dq6, dq7, dq8, dq9, dq10 = dq_leg_l[0], dq_leg_l[1], dq_leg_l[2], dq_leg_l[3], dq_leg_l[4]
    ...
    R2 = MF.Rx(+PI_2) @ MF.Rz(q2 - PI_2)
    X2[0:3, 0:3] = R2
    X2[3:6, 3:6] = R2
    X2[0:3, 3:6] = r_12_hat @ R2

    R3 = MF.Rx(-PI_2) @ MF.Rz(q3)
    X3[0:3, 0:3] = R3
    X3[3:6, 3:6] = R3
    X3[0:3, 3:6] = r_23_hat @ R3

    R4 = MF.Rx(+PI_2) @ MF.Rz(q4)
    X4[0:3, 0:3] = R4
    X4[3:6, 3:6] = R4
    ...
    R7 = MF.Rx(+PI_2) @ MF.Rz(q7 - PI_2)
    X7[0:3, 0:3] = R7
    X7[3:6, 3:6] = R7
    X7[0:3, 3:6] = r_67_hat @ R7

    R8 = MF.Rx(-PI_2) @ MF.Rz(q8)
    X8[0:3, 0:3] = R8
    X8[3:6, 3:6] = R8
    X8[0:3, 3:6] = r_78_hat @ R8

    R9 = MF.Rx(+PI_2) @ MF.Rz(q9)
    X9[0:3, 0:3] = R9
    X9[3:6, 3:6] = R9
