...
def main_loop():
    ...
    read_write_frequency   = 50  # read/write at 50 Hz
    check_thread_frequency = 10  # check thread error at 10 Hz

    read_write_duration    = 1. / read_write_frequency
    check_thread_duration  = 1. / check_thread_frequency

    print("======= The DXL Actuator Communication Thread is running at", read_write_frequency, "Hz... =======")
    ...
    while True:
        ...
        if not thread_run and elapsed_time > 1:
            ...
            MM.THREAD_STATE.set({'dxl': np.array([1])}, opt='update')  # thread is running
            thread_run = True
...
if __name__ == "__main__":
    ...
    try:
        ...
        MM.THREAD_STATE.set({'dxl': np.array([0])}, opt='update')  # thread is stopped
    except (Exception, KeyboardInterrupt) as error:
        print(error)
        print(colored('THREAD IN ERROR! TERMINATE NOW!', 'red'))
        MM.THREAD_STATE.set({'dxl': np.array([2])}, opt='update')  # thread in error
    finally:
        dc.torque_enable_all(0)