...
class BRUCE_IN_DANGER(Exception):
    """
    Raised when BRUCE in danger
    """
    pass
...
class BRUCE:
    ...
    def update_sense_status(self):
        ...
        self.imu_rot_matrix = sense_data['imu_rot_matrix']
    ...
    def update_leg_status(self, torque=False):
        ...
        if torque:
            ...
            self.joint[HIP_PITCH_R]['u']   = u[1]
            self.joint[HIP_ROLL_R]['u']    = u[2]
            self.joint[KNEE_PITCH_R]['u']  = u[3]
            self.joint[ANKLE_PITCH_R]['u'] = u[4]
            self.joint[HIP_YAW_L]['u']     = u[5]
            self.joint[HIP_PITCH_L]['u']   = u[6]
            self.joint[HIP_ROLL_L]['u']    = u[7]
            self.joint[KNEE_PITCH_L]['u']  = u[8]
            self.joint[ANKLE_PITCH_L]['u'] = u[9]

    def update_leg_commands(self, torque=False):
        """
        Get leg commands from shared memory.
        """
        leg_commands = MM.LEG_COMMAND.get()
        q  = leg_commands['goal_positions']
        dq = leg_commands['goal_velocities']

        # right leg
        self.joint[HIP_YAW_R]['q_goal']     = q[0]
        self.joint[HIP_PITCH_R]['q_goal']   = q[1]
        self.joint[HIP_ROLL_R]['q_goal']    = q[2]
        self.joint[KNEE_PITCH_R]['q_goal']  = q[3]
        self.joint[ANKLE_PITCH_R]['q_goal'] = q[4]

        self.joint[HIP_YAW_R]['dq_goal']     = dq[0]
        self.joint[HIP_PITCH_R]['dq_goal']   = dq[1]
        self.joint[HIP_ROLL_R]['dq_goal']    = dq[2]
        self.joint[KNEE_PITCH_R]['dq_goal']  = dq[3]
        self.joint[ANKLE_PITCH_R]['dq_goal'] = dq[4]

        # left leg
        self.joint[HIP_YAW_L]['q_goal']     = q[5]
        self.joint[HIP_PITCH_L]['q_goal']   = q[6]
        self.joint[HIP_ROLL_L]['q_goal']    = q[7]
        self.joint[KNEE_PITCH_L]['q_goal']  = q[8]
        self.joint[ANKLE_PITCH_L]['q_goal'] = q[9]

        self.joint[HIP_YAW_L]['dq_goal']     = dq[5]
        self.joint[HIP_PITCH_L]['dq_goal']   = dq[6]
        self.joint[HIP_ROLL_L]['dq_goal']    = dq[7]
        self.joint[KNEE_PITCH_L]['dq_goal']  = dq[8]
        self.joint[ANKLE_PITCH_L]['dq_goal'] = dq[9]

        if torque:
            u = leg_commands['goal_torques']
            self.joint[HIP_YAW_R]['u_goal']     = u[0]
            self.joint[HIP_PITCH_R]['u_goal']   = u[1]
            self.joint[HIP_ROLL_R]['u_goal']    = u[2]
            self.joint[KNEE_PITCH_R]['u_goal']  = u[3]
            self.joint[ANKLE_PITCH_R]['u_goal'] = u[4]
            self.joint[HIP_YAW_L]['u_goal']     = u[5]
            self.joint[HIP_PITCH_L]['u_goal']   = u[6]
            self.joint[HIP_ROLL_L]['u_goal']    = u[7]
            self.joint[KNEE_PITCH_L]['u_goal']  = u[8]
            self.joint[ANKLE_PITCH_L]['u_goal'] = u[9]

    def set_command_leg_positions(self):
        """
        Set command leg joint positions to shared memory.
        """
        commands = {'BEAR_enable': np.array([1]),
    ...
    def set_command_leg_torques(self):
        ...
        commands = {'BEAR_enable': np.array([1]),
                    ...
                    'goal_torques': np.array([self.joint[HIP_YAW_R]['u_goal'],
                                              self.joint[HIP_PITCH_R]['u_goal'],
                                              self.joint[HIP_ROLL_R]['u_goal'],
                                              self.joint[KNEE_PITCH_R]['u_goal'],
                                              self.joint[ANKLE_PITCH_R]['u_goal'],
                                              self.joint[HIP_YAW_L]['u_goal'],
                                              self.joint[HIP_PITCH_L]['u_goal'],
                                              self.joint[HIP_ROLL_L]['u_goal'],
                                              self.joint[KNEE_PITCH_L]['u_goal'],
                                              self.joint[ANKLE_PITCH_L]['u_goal'],
                                              ])
                    }
        MM.LEG_COMMAND.set(commands)
    ...
    def calculate_leg_FK(self, leg, location, q):
        ...
        return kin.leg_FK(leg, option, q)
    
    def solve_leg_IK(self, leg, x, p, q0=None):
        """
        Solve leg inverse kinematics
        """
        if leg == "right":
            leg = +1
            joint_list = [0, 1, 2, 3, 4]
        elif leg == "left":
            leg = -1
            joint_list = [5, 6, 7, 8, 9]

        if q0 is None:
            q0 = np.zeros(5)

        q = kin.leg_IK(leg, 0, x, p, q0)

        num_sol = 2
        joint_limit_check_list = [True]  * num_sol
        orientation_check_list = [False] * num_sol
        position_check_list    = [False] * num_sol
        for idx in range(num_sol):
            R1, p1 = self.calculate_leg_FK(leg, "ankle", q[idx, :])
            x1 = R1[0:3, 0]
            
            if np.linalg.norm(x1 - x) < 1e-3:
                orientation_check_list[idx] = True
                
                for jdx, joint_id in enumerate(joint_list):
                    if q[idx, jdx] < LEG_JOINT_POS_MIN[joint_id]:
                        q[idx, jdx] = LEG_JOINT_POS_MIN[joint_id]
                        joint_limit_check_list[idx] = False
                    if q[idx, jdx] > LEG_JOINT_POS_MAX[joint_id]:
                        q[idx, jdx] = LEG_JOINT_POS_MAX[joint_id]
                        joint_limit_check_list[idx] = False
        ...
        for idx in range(num_sol):
            if orientation_check_list[idx] and position_check_list[idx]:
                print(colored("IK Exceeds Joint Limit!", 'yellow'))
                return q[idx, :]
            elif orientation_check_list[idx]:
                print(colored("IK Out of Workspace!", 'yellow'))
                return q0
        
        print(colored("IK Infeasible!", 'yellow'))
        return q0

    @staticmethod
    def stop_robot():
        BEAR_commands = {'BEAR_enable': np.array([0])}
        MM.LEG_COMMAND.set(BEAR_commands)

        DXL_commands = {'DXL_enable': np.array([0])}
